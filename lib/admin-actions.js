...
async function handleSupportReply(body) {
  ...
  const ticket = ticketId && isSupabaseConfigured()
    ? await getSupportTicket(ticketId).catch(() => null)
    : null;
  const metadata = ticket && ticket.metadata && typeof ticket.metadata === 'object' ? ticket.metadata : {};
  const chatEnabled = metadata.channel === 'chat' || Boolean(ticket && ticket.user_id);
  const result = await sendSupportReplyEmail({
    to: email,
    subject: clean(body.subject, 180) || (metadata.support_number ? `Svar från Versen support ${metadata.support_number}` : 'Svar från Versen support'),
    message,
  });

  let updatedTicket = null;
  if (ticket && isSupabaseConfigured()) {
    updatedTicket = await appendSupportMessage(ticket.id, {
      from: 'admin',
      name: 'Versen support',
      email: process.env.VERSEN_SUPPORT_EMAIL || process.env.VERSEN_EMAIL_FROM || '',
      message,
      attachments: [],
    }, {
      status: chatEnabled ? 'väntar på kund' : (ticket.status || 'pågående'),
  ...
  await safeLog({
    ...
    metadata: { ok: result.ok, status: result.status || null, chat: chatEnabled },
  });

  return {
    status: result.ok || updatedTicket ? 200 : 502,
    body: {
      ok: Boolean(result.ok || updatedTicket),
      status: result.ok
        ? (chatEnabled ? 'Svaret är skickat i chatten och via email.' : 'Svaret är skickat.')
        : (updatedTicket ? 'Svaret syns i chatten men emailnotisen kunde inte skickas.' : 'Kunde inte skicka supportsvaret.'),
      emailStatus: result,
