...
const { ensureStripePaymentMethodDomains, stripePublishableKey, stripeRequest } = require('./stripe');

const FREE_CHECKOUT_TEST_CODE = '63630325';
const STANDARD_SHIPPING_ORE = 5500;
...
function applyFreeCheckoutTestDiscount(validation, discountCode) {
  if (!isFreeCheckoutTestCode(discountCode)) {
    return validation;
  }

  const originalSubtotal = Number(validation.original_subtotal || validation.subtotal) || 0;
  const freeDiscountCodes = [
    ...(validation.discount_codes || []).filter((item) => item && String(item.code || '').toLowerCase() !== FREE_CHECKOUT_TEST_CODE),
    { code: FREE_CHECKOUT_TEST_CODE, applicable: true },
  ];

  return {
    ...validation,
    discount_codes: freeDiscountCodes,
    subtotal: 0,
    discount: originalSubtotal + Number(validation.shipping || 0),
    shipping: 0,
    tax: 0,
    total: 0,
    free_checkout: true,
    summary: {
      ...validation.summary,
      subtotal: formatSekOre(0),
      discount: formatSekOre(originalSubtotal + Number(validation.shipping || 0)),
      shipping: formatSekOre(0),
      tax: formatSekOre(0),
      total: formatSekOre(0),
    },
  };
}
...
async function validateCheckout({ items, discountCode, session }) {
  ...
  const shippingOre = STANDARD_SHIPPING_ORE;
  ...
  const blockedItem = normalizedItems.find((item) => item.handle === 'medlemskap');
  if (blockedItem) {
    const error = new Error('Medlemskap köps separat och kan inte ligga i produktcheckout.');
    error.status = 400;
    throw error;
  }

  return applyFreeCheckoutTestDiscount({
    cart_id: cart.id,
...
async function createPaymentIntent({ req, validation, contact, shippingAddress, session }) {
  ...
  await ensureStripePaymentMethodDomains([req && req.headers && req.headers.host]).catch(() => {});
...
async function createFreeCheckoutDraft({ req, validation, contact, shippingAddress, session }) {
  const checkoutId = `free_${crypto.randomUUID().replace(/-/g, '').slice(0, 24)}`;
...
async function syncOrderToShopify(order) {
  ...
  const freeCheckout = String(order.stripe_payment_intent_id || '').startsWith('free_');
  const freeCheckoutDiscount = order.items.reduce((sum, item) => sum + (Number(item.total_price) || 0), 0);
  ...
  const response = await fetch(`https://${domain}/admin/api/${apiVersion}/orders.json`, {
    ...
    body: JSON.stringify({
      order: {
        ...
        tags: freeCheckout ? 'versen_internal_checkout,free_test_checkout' : 'versen_internal_checkout,stripe_paid',
        line_items: lineItems,
        discount_codes: freeCheckout
          ? [{ code: FREE_CHECKOUT_TEST_CODE, amount: (freeCheckoutDiscount / 100).toFixed(2), type: 'fixed_amount' }]
          : undefined,
        shipping_lines: order.shipping > 0
          ? [{
            title: 'Frakt',
            code: 'standard_shipping',
            price: (order.shipping / 100).toFixed(2),
          }]
          : undefined,
        shipping_address: {
        ...
        transactions: [
          {
            ...
            gateway: freeCheckout ? 'manual' : 'stripe',
          },
        ],
        note_attributes: [
          { name: 'Versen order id', value: order.id },
          { name: freeCheckout ? 'Versen gratis checkout' : 'Stripe PaymentIntent', value: order.stripe_payment_intent_id },
          { name: 'Versen checkout', value: freeCheckout ? 'Intern gratis testcheckout' : 'Intern Stripe checkout' },
        ],
      },
    }),
  });
...
async function fulfillFreeCheckoutDraft(checkoutId) {
  if (!String(checkoutId || '').startsWith('free_')) {
    const error = new Error('Gratis checkout-referens saknas');
    error.status = 400;
    throw error;
  }

  const existing = await getOrderByPaymentIntent(checkoutId);
  if (existing) {
    return existing;
  }

  const draft = await getDraft(checkoutId);
  if (!draft) {
    const error = new Error('Orderunderlag saknas för gratis checkout');
    error.status = 409;
    throw error;
  }

  if (Number(draft.total) !== 0) {
    const error = new Error('Gratis checkout kan bara slutföras med total 0 kr');
    error.status = 409;
    throw error;
  }
  ...
  const order = {
    ...
    stripe_payment_intent_id: checkoutId,
    payment_status: 'paid',
    order_status: 'free_test_pending_shopify_sync',
    shopify_order_id: '',
    order_number: `#V${Date.now().toString().slice(-6)}`,
    created_at: new Date().toISOString(),
  };
  ...
  if (synced.ok) {
    order.shopify_order_id = synced.orderId;
    order.order_number = synced.orderName || order.order_number;
    order.order_status = 'free_test_synced_shopify';
  } else {
    order.shopify_sync_error = synced;
  }
