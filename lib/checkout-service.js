...
function checkoutUserId(session = {}, email = '') {
  if (session.authenticated && session.customer && session.customer.id) {
    return session.customer.id;
  }

  const normalizedEmail = String(email || '').trim().toLowerCase();
  if (normalizedEmail) {
    return `guest:${normalizedEmail}`;
  }
...
async function getSession(req) {
  ...
  return { customerAccessToken, session };
}
...
async function validateCheckout({ items, discountCode, customerAccessToken, session, contact }) {
  ...
  if ((customer && customer.email) || contactEmail) {
    buyerIdentity.email = customer && customer.email ? customer.email : contactEmail;
  }
...
async function createPaymentIntent({ req, validation, contact, shippingAddress, session }) {
  ...
  const userId = checkoutUserId(session, contact.email);
