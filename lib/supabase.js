...
function supportMessageRecord(message = {}) {
  return {
    id: message.id || `msg_${Date.now()}_${Math.random().toString(16).slice(2, 8)}`,
    from: message.from || 'customer',
    name: message.name || null,
    email: String(message.email || '').toLowerCase() || null,
    message: message.message || message.body || '',
    attachments: Array.isArray(message.attachments) ? message.attachments.slice(0, 3) : [],
    created_at: message.created_at || new Date().toISOString(),
  };
}

function supportTicketUpdateRecord(update = {}) {
  const body = {
    updated_at: new Date().toISOString(),
  };

  ['status', 'category', 'priority', 'unread', 'messages', 'metadata', 'message'].forEach((key) => {
...
async function getSupportTicket(ticketId) {
  const rows = await supabaseRequest(`${tableName('SUPABASE_SUPPORT_TICKETS_TABLE', 'support_tickets')}?id=eq.${encode(ticketId)}&limit=1`);
  return Array.isArray(rows) ? rows[0] || null : null;
}

async function listSupportTicketsForCustomer(userId, email, limit = 50) {
  const filters = [];
  ...
  const rows = await supabaseRequest(`${tableName('SUPABASE_SUPPORT_TICKETS_TABLE', 'support_tickets')}?or=(${filters.join(',')})&order=updated_at.desc&limit=${Number(limit) || 50}`);
  return Array.isArray(rows) ? rows : [];
}

async function patchSupportTicket(ticketId, update) {
  const rows = await supabaseRequest(`${tableName('SUPABASE_SUPPORT_TICKETS_TABLE', 'support_tickets')}?id=eq.${encode(ticketId)}`, {
    method: 'PATCH',
    headers: { Prefer: 'return=representation' },
    body: JSON.stringify(supportTicketUpdateRecord(update)),
  });
...
async function appendSupportMessage(ticketId, message, options = {}) {
  ...
  const nextMessage = supportMessageRecord(message);
  const metadata = ticket.metadata && typeof ticket.metadata === 'object' ? ticket.metadata : {};
  const fromAdmin = nextMessage.from === 'admin';
  const nextMetadata = {
    ...metadata,
    channel: metadata.channel || options.channel || 'chat',
    latest_message_at: nextMessage.created_at,
    customer_unread: fromAdmin,
    admin_unread: !fromAdmin,
  };

  if (options.admin_read !== undefined) nextMetadata.admin_unread = !options.admin_read;
  if (options.customer_read !== undefined) nextMetadata.customer_unread = !options.customer_read;

  return patchSupportTicket(ticketId, {
    messages: [...existingMessages, nextMessage],
    message: ticket.message || nextMessage.message,
    unread: !fromAdmin,
    status: options.status || ticket.status,
...
async function updateOrderItems(orderId, update) {
  const body = {
    items: Array.isArray(update.items) ? update.items : [],
  };

  if (update.subtotal !== undefined) body.subtotal = Number(update.subtotal) || 0;
  if (update.total !== undefined) body.total = Number(update.total) || 0;

  return patchAdminTable('SUPABASE_ORDERS_TABLE', 'orders', orderId, body);
}
...
async function clearAbandonedCheckout(checkoutId) {
  return patchAdminTable('SUPABASE_ABANDONED_CHECKOUTS_TABLE', 'abandoned_checkouts', checkoutId, {
    status: 'cleared',
    metadata: {
