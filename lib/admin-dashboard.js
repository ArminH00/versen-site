...
function normalizeShopifyOrder(order) {
  ...
  return {
    source: 'shopify',
    id: order.id,
    shopifyOrderId: shopifyNumericId(order.id),
    name: order.name,
...
function normalizeTicket(row) {
  ...
  const supportNumber = (() => {
    const current = String(metadata.support_number || '').trim().toUpperCase();
    if (/^V\d{5}$/.test(current)) return current;
    const text = String(row.id || current || 'support');
    let hash = 0;
  ...
  return {
    ...
    adminUnread: metadata.admin_unread !== undefined ? Boolean(metadata.admin_unread) : Boolean(row.unread),
    customerUnread: Boolean(metadata.customer_unread),
    priority: row.priority || 'normal',
    message: row.message || '',
    messages,
    metadata,
    channel: metadata.channel || (row.user_id ? 'chat' : 'email'),
    supportNumber,
    latestMessageAt: metadata.latest_message_at || (latest && latest.created_at) || row.updated_at || row.created_at,
    createdAt: row.created_at,
    updatedAt: row.updated_at,
  };
}
...
module.exports = async function handler(req, res) {
  ...
  const supabaseShopifyIds = new Set(supabaseOrderList.map((order) => String(order.shopifyOrderId || '')).filter(Boolean));
  const orders = [
    ...shopifyOrderList.filter((order) => !order.shopifyOrderId || !supabaseShopifyIds.has(String(order.shopifyOrderId))),
    ...supabaseOrderList,
  ]
  ...
  const packingStatuses = ['paid_pending_shopify_sync', 'paid_synced_shopify', 'unfulfilled', 'open', 'pending', 'mottagen', 'plockas'];
  const shippedStatuses = ['fulfilled', 'shipped', 'delivered', 'skickad', 'levererad'];
  ...
  sendJson(res, 200, {
    ...
    stats: {
      ...
      unreadSupportMessages: tickets.filter((ticket) => ticket.adminUnread || ticket.unread || ticket.status === 'unread').length,
    ...
    settings: {
      orderStatuses: ['mottagen', 'plockas', 'plockad', 'skickad', 'levererad', 'makulerad', 'återbetald', 'retur'],
      supportCategories: ['olästa', 'nytt', 'pågående', 'väntar på kund', 'löst', 'stängt', 'returer', 'övrigt'],
      emailTemplates: [
        'email_verification',
        'account_created',
        'password_reset',
