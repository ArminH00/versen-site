...
const { ensureStripePaymentMethodDomains, stripeRequest } = require('./stripe');
...
function subscriptionPriceDetails(subscription = {}) {
  ...
  const plan = item && item.plan ? item.plan : null;

  return {
    amount: Number((price && price.unit_amount) || (plan && plan.amount) || 0) || 0,
    currency: String((price && price.currency) || (plan && plan.currency) || 'sek').toLowerCase(),
    interval: (price && price.recurring && price.recurring.interval) || (plan && plan.interval) || null,
    price_id: (price && price.id) || (plan && plan.id) || null,
  };
}
...
async function createMembershipSubscription({ customer, plan, inviteCode }) {
  ...
  await ensureStripePaymentMethodDomains().catch(() => {});
...
async function createStripeBillingPortalSession({ subscriptionId, returnUrl }) {
  const row = await getSubscriptionByStripeId(subscriptionId);

  if (!row || !row.stripe_customer_id) {
    const error = new Error('Ingen Stripe-kund hittades för medlemskapet.');
    error.status = 404;
    throw error;
  }

  const body = new URLSearchParams();
  let fallbackOrigin = 'https://versen.se';
  try {
    fallbackOrigin = new URL(process.env.VERSEN_SITE_URL || fallbackOrigin).origin;
  } catch (error) {
    fallbackOrigin = 'https://versen.se';
  }

  body.set('customer', row.stripe_customer_id);
  body.set('return_url', returnUrl || `${fallbackOrigin}/installningar`);
  ...
  if (!session || !session.url) {
    const error = new Error('Stripe skapade ingen portal-länk.');
    error.status = 502;
    throw error;
  }
