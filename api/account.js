...
async function sendCustomerSupportList(req, res, session) {
  const customer = session && session.authenticated ? session.customer : null;
  if (!customer) {
  ...
  const tickets = rows.filter(isChatTicket).filter((ticket) => isCustomerTicket(ticket, customer)).map(normalizeSupportTicket);
  sendJson(res, 200, { tickets });
}

async function sendCustomerSupportTicket(req, res, session, ticketId) {
  const customer = session && session.authenticated ? session.customer : null;
  if (!customer) {
    sendJson(res, 401, { error: 'Logga in för att öppna supportchatten.' });
    return;
  ...
  sendJson(res, 200, { ticket: normalizeSupportTicket(ticket) });
}

async function sendCustomerSupportMessage(req, res, body) {
  ...
  if (!customer) {
    sendJson(res, 401, { error: 'Logga in för att skriva i supportchatten.' });
    return;
  }

  const ticketId = clean(body.ticketId, 160);
  const message = clean(body.message, 3000);
  const attachments = safeArray(body.attachments).map(normalizeSupportAttachment).filter(Boolean).slice(0, 3);
  ...
  sendJson(res, 200, { ticket: normalizeSupportTicket(updated) });
}
...
module.exports = async function handler(req, res) {
  if (req.method === 'GET') {
    ...
    if (url.searchParams.get('mode') === 'support') {
      const ticketId = clean(url.searchParams.get('ticket'), 160);
      if (ticketId) {
        await sendCustomerSupportTicket(req, res, session, ticketId);
      } else {
        await sendCustomerSupportList(req, res, session);
      }
      return;
    }
  ...
  if (body.action === 'support_message') {
    await sendCustomerSupportMessage(req, res, body);
    return;
  }
